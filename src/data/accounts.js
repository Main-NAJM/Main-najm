...
export const listAccounts = () => readAccounts().sort((a, b) => (b.lastSignInAt || 0) - (a.lastSignInAt || 0));
export const hasAccounts = () => readAccounts().length > 0;
/* ------------------------------------------------------------ تلبيد كلمة المرور */
const subtle = () => {
    const value = globalThis.crypto?.subtle;
    if (!value) {
        throw new Error('هذا المتصفّح لا يدعم إنشاء حساب محفوظ بكلمة مرور. تابع بدون حساب، أو افتح التطبيق من رابطه الآمن (https).');
    }
    return value;
};
...
const derive = async (password, salt) => {
    ...
    const bits = await subtle().deriveBits({ name: 'PBKDF2', salt: salt, iterations: ITERATIONS, hash: 'SHA-256' }, key, 256);
...
export const findAccount = (ident) => readAccounts().find((account) => account.ident === ident) ?? null;
/* ----------------------------------------------------------------- الجلسة */
...
export const getAccount = (uid) => readAccounts().find((account) => account.uid === uid) ?? null;
/**
...
export const signInAccount = async (identInput, password) => {
    ...
    if (!equals(hash, account.hash))
        throw rejected;
    const updated = { ...account, lastSignInAt: Date.now() };
...
export const patchAccount = (uid, patch) => {
    const accounts = readAccounts();
...
export const changePassword = async (uid, currentPassword, nextPassword) => {
    ...
    if (!equals(current, account.hash))
        throw new Error('كلمة المرور الحالية غير صحيحة.');
    const salt = globalThis.crypto.getRandomValues(new Uint8Array(16));
    const hash = await derive(nextPassword, salt);
    writeAccounts(readAccounts().map((row) => row.uid === uid ? { ...row, salt: toBase64(salt), hash } : row));
};
