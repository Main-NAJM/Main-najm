...
const foot = (profile, note) => `
  <div class="doc__foot">
    <span>${escapeHtml(note ?? 'شكراً لتعاملكم معنا')}</span>
    <span>${escapeHtml(profile.businessName || 'ورشتي')}</span>
  </div>`;
const money = (value, profile) => escapeHtml(formatMoney(value, profile.currency));
/* ---------------------------------------------------------------- فاتورة */
export const invoiceTitle = (order) => `فاتورة ${order.title || 'طلبية'}`;
export const buildInvoice = (order, profile) => {
...
/* --------------------------------------------------------- كشف الطلبيات */
export const buildOrdersReport = (orders, profile, subtitle) => {
...
/* ------------------------------------------------------------ سجل الديون */
export const buildDebtsReport = (debts, profile, subtitle) => {
...
export const buildDebtStatement = (debt, profile) => {
    const t = debtTotals(debt);
...
/* ---------------------------------------------------------- سجل التسعير */
export const buildCalculationSheet = (calc, profile) => {
...
export const buildCalculationsReport = (calculations, profile) => {
...
/* ----------------------------------------------------------- عرض سعر منتج */
export const buildProductQuote = (product, dims, result, profile) => {
    const dimText = [
    ...
    <div class="block">
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td>${product.basis === 'unit'
        ? 'قيمة المادة'
        : product.basis === 'frame'
            ? `البروفيل — المحيط (${escapeHtml(formatNumber(result.measure))} م.ط × ${money(product.unitPrice, profile)})`
            : `قيمة المادة (${escapeHtml(formatNumber(result.measure))} ${escapeHtml(result.measureUnit)} × ${money(product.unitPrice, profile)})`}</td>
