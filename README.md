...
مجلد `beauty/` يحوي متجر **أم آية** من صفحة واحدة، بقسمين: **مجموعة التجميل** مع تصفية
...
```bash
npm run beauty         # ثم افتح http://localhost:8081
npm run beauty -- 3000 # منفذ مخصّص
```
...
`<script>` داخل `beauty/index.html`، وما لم يُضبط منها يظهر تنبيهاً في أعلى الصفحة بدل
رابط معطوب. الشرح الكامل في [`beauty/README.md`](beauty/README.md).
